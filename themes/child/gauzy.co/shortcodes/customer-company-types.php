...
function gauzy_customer_company_type_custom_post_type() {
	$labels = array(
...
add_action( 'init', 'gauzy_customer_company_type_custom_post_type', 0 );

/* Create Shortcode to display Customer Company Types archives */

function gauzy_company_types(){
    global $post;
    ob_start();
    $recent = new WP_Query(); 
    $query = array( 
...
add_shortcode( 'gauzy_company_types', 'gauzy_company_types' );