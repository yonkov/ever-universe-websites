...
function ever_co_remove_p_filter() {
    global $post;

    if ( 'Business Verticals' == $post->post_type )
        remove_filter( 'the_content', 'wpautop' );
}
add_action( 'wp', 'ever_co_remove_p_filter' );

/* Add single post title as body class */

// Add specific CSS class by filter
add_filter( 'body_class', 'ever_co_class_names' );
function ever_co_class_names( $classes ) 
{
	global $post;
...
add_action( 'admin_menu', 'ever_co_create_post_meta_box' );
add_action( 'save_post', 'ever_co_save_post_meta_box', 10, 2 );

function ever_co_create_post_meta_box() {
    add_meta_box( 'my-meta-box', 'Second Excerpt', 'ever_co_post_meta_box', 'Company Types', 'normal', 'high' );
}

function ever_co_post_meta_box( $object, $box ) { ?>
    <p>
        ...
        <input type="hidden" name="ever_co_meta_box_nonce" value="<?php echo wp_create_nonce( plugin_basename( __FILE__ ) ); ?>" />
    </p>
<?php }

function ever_co_save_post_meta_box( $post_id, $post ) {

    if ( !wp_verify_nonce( $_POST['ever_co_meta_box_nonce'], plugin_basename( __FILE__ ) ) )
        return $post_id;
