...
function addDropdowns() {
    ...
    // Filtering to only mobile dropdowns
    dropdowns = dropdowns.filter(dropDown => dropDown.id ? '' : dropDown);
    // Assigning elemets to object 
    var dropdownList = dropdowns.reduce(function (acc, cur, i) {
        acc[i] = { 'element': cur, 'isOppened': false };
        return acc
    }, [])
    // Adding an eventlistener to mobile dropdowns
    dropdowns.forEach(dropD => dropD.addEventListener('click', (e) => {
        e.stopPropagation();
        // Check if clicked element is a dropdown element
