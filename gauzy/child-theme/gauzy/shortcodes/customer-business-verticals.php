...
function gauzy_customer_business_vertical_custom_post_type() {
	$labels = array(
...
add_action( 'init', 'gauzy_customer_business_vertical_custom_post_type', 0 );

/* Create Shortcode to display Customer Business Verticals archives */

function gauzy_business_verticals(){
    global $post;
    ob_start();
    $recent = new WP_Query(); 
    $query = array( 
...
add_shortcode( 'gauzy_business_verticals', 'gauzy_business_verticals' );